...
class DrawMapLines(object):
    def __init__(self, data, width, height,color):
        self.log = []
        ...
        with open("../map.grp", "rb") as f:
            self.map = list(f.read())
        with open("../../Resources/dsbuf.dat", "rb") as f:  # content loaded from debug.exe: d ds:0
            self.DS_BUF = list(f.read())
