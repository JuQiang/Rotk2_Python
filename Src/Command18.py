...
class Command18(object):
    def __init__(self):
        self.commands = [Helper.GetBuiltinText(0x89DE,0x89E1),Helper.GetBuiltinText(0x89EB,0x89EE),Helper.GetBuiltinText(0x89F8,0x89FB)]
        self.palette_no = 5
    ...
    def GetPassenger(self):
        ...
        province_no = RoTK2.GetRulerProvinceNo(current_ruler_province)
    ...
    def ShowTalk(self, passenger_no, talk):
        if passenger_no<3:
            if passenger_no<2:
                passenger_name = Helper.GetBuiltinText(Data.DSBUF[0x8710+2*passenger_no+1]*256+Data.DSBUF[0x8710+2*passenger_no+0])
            ...
            passenger_face = Data.DSBUF[0x871C+2*passenger_no+1]*256+Data.DSBUF[0x871C+2*passenger_no+0] - 1
        else:
            advisor = RoTK2.GetAdvisor()
            passenger_name = RoTK2.GetOfficerName(advisor.Offset)
        ...
        pygame.draw.rect(bmp,(255,255,255),(100,40,200,80),border_radius=5)
        left = 90
        top = 80
    ...
    def Start(self,province_no):
        ...
        Helper.ClearInputArea()
        if RoTK2.GetAdvisorProvince()!=province_no:
        ...
        Helper.ShowCommandsInInputArea(self.commands,3,palette_no=self.palette_no,width=70,commands_color=self.commands_color)

        cmd = Helper.GetInput(Helper.GetBuiltinText(0x5342) + "(1-{0})?".format(len(self.commands)), row=1,required_number_min=1, required_number_max=3, allow_enter_exit=True)
        ...
        if cmd==1:
            ...
            if advisor_offset==0 or RoTK2.GetAdvisorProvince()!=province_no:
                Helper.ShowDelayedText(Helper.GetBuiltinText(0x89BA),palette_no=7)
                return

            if ((Data.BUF[ruler_offset+7] & 0x20)==0x20) or (Data.BUF[advisor_offset+0x0B]<=random.randint(0,100)):
    ...
    def IsNeighborCanAttack(self,neighbor):
        ...
        province = RoTK2.GetProvinceBySequence(neighbor)

        if province.RulerNo == Helper.GetCurrentRulerNo() or province.RulerNo == 0xFF or province.WarRulerNo != 0xFF:
            return False
        return True
    ...
    def AdviceWar2(self,neighbors):
        me = RoTK2.GetProvinceBySequence(RoTK2.GetCurrentProvinceNo())
        for neighbor in neighbors:
            if self.IsNeighborCanAttack(neighbor):
                province = RoTK2.GetProvinceBySequence(neighbor)
    ...
    def AdviceWar4(self,neighbors):
        ...
        for neighbor in neighbors:
            if self.IsNeighborCanAttack(neighbor):
                province = RoTK2.GetProvinceBySequence(neighbor)
                if len(province.OfficerList)>=0x14:
                    return neighbor
    ...
    def AdviceOfficer(self):
        ...
        for ruler in RoTK2.GetRulerList():
            if ruler.RulerSelf.Offset==RoTK2.GetCurrentRulerOfficerOffset():
                continue
        ...
        province_list = RoTK2.GetProvincesByRulerNo(ruler.No)
        for province in province_list:
            for officer in province.OfficerList:
        ...
        if len(officer_list)>0:
            text = [Data.DSBUF[0x88C7]*256+Data.DSBUF[0x88C6],Data.DSBUF[0x88C9]*256+Data.DSBUF[0x88C8],Data.DSBUF[0x88CB]*256+Data.DSBUF[0x88CA]]
            choice2 = random.randint(0, len(officer_list)-1)
            name = RoTK2.GetOfficerName(officer_list[choice2].Offset)
    ...
    def AdviceSpy(self):
        ...
        province_list = RoTK2.GetProvincesByRulerNo(Helper.GetCurrentRulerNo())
        for province in province_list:
            for officer in province.OfficerList:
    ...
    def Simahui(self):
        ...
        for ruler in ruler_list:
            ...
            if index==ruler1:
                ruler1_name = Helper.GetRulerName(self.province_no)
                ruler2 = Data.BUF[ruler.Offset+0x09]
                if ruler2!=0xFF:
                    ruler2_name = RoTK2.GetOfficerName(ruler_list[ruler2].RulerSelf.Offset)
                    break
        ...
        if len(ruler1_name)>0:
            ...
            for i in range(0,41):
                ...
                if (Data.BUF[province_offset+5]*256+Data.BUF[province_offset+4]>0) or (Data.BUF[province_offset+7]*256+Data.BUF[province_offset+6]>0):
                    choice = random.randint(0, 100)
                    if choice < 50:
    ...
    def GetOfficerSpecialList(self,choice):
        ...
        current_ruler_province = Data.BUF[0x339B] * 256 + Data.BUF[0x339A]
        province_list = RoTK2.GetProvincesByRulerNo(Data.BUF[current_ruler_province+0x10])
        for p in province_list:
            for o in p.OfficerList:
                if choice==0:
                    if (Data.BUF[o.Offset+0x02]>>4)%2==1:#dead
                        special_list.append(RoTK2.GetOfficerName(o.Offset))
                elif choice==1:
